...
export default {
    ...
    methods: {
      ...
      submitForm(formName) {
        ...
        console.log(data);
        if(this.checked){
          this.$http('/user/user/register',data).then(function(res){
              var d=res.data;
              if(d.code==1000){
      ...
      handleYanzheng(e){
        ...
        var regExp = /^1[3|5|8|7][0-9]{9}$/;
       if (this.ruleForm2.phone&&regExp.test(this.ruleForm2.phone) === true)
       {
          this.$http('/user/sms/send',{mobile:this.ruleForm2.phone,type:1}).then(function(res){
            var d=res.data;
            if(d.code==201){
                console.log(222);
