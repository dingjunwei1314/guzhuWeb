   ...
   <transition
    ...
    <div class="successAlert"  v-show="isshow" :style="{width:AlertWidth+'px',height:AlertHeight+'px',marginLeft:'-'+AlertWidth/2+'px',marginTop:'-'+AlertHeight/2+'px'}">
      <div v-show='page==1' class='bg'>
        ...
        <p style='margin:225px 0 30px 40px;width:310px ;font-size:18px;color:#fff;line-height:30px'>测试时间为90秒，正确率达到90%即可 通过测试，准备好了吗 </p>
      ...
      <div v-show='page==3' id='success'>
        <p class="tit">
        <img @click="close" src="./img/close.png">
        </p>
        <h4>打字测试：<span>4.0</span>分</h4>
        <h1>恭喜您，通过了打字测试</h1>
        <div style='text-align:center'>
            <el-button class='btn'  @click.prevent='close'>完成</el-button>
        </div>
      </div>
      <div v-show='page==4' id='success'>
        <p class="tit">
        <img @click="close" src="./img/close.png">
        </p>
        <h4>打字测试：<span style='color:#fb5e4b'>4.0</span>分</h4>
        <h1>差一点就通过了，请再接再厉噢。</h1>
        <div style='text-align:center'>
            <el-button class='btn1'  @click.prevent='close'>休息一下再测</el-button>
            <el-button class='btn'  @click.prevent='sure'>不信邪再测一次</el-button>
        </div>
      </div>
    </div>  
